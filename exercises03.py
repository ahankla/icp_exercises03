...
def rk4b3(xdot, vdot, x0, v0, m, h, n):
    """Runge-Kutta for 3-body problem
        Same as rk4, but integrate each left-hand side
    ...
    xt = np.zeros((n+1, 2, 3)); #n+1 time steps, 2 coords (x, y), 3 bodies (0, 1, 2)
    vt = np.zeros((n+1, 2, 3));
    ...
    for i in range(1, n + 1):
        ...
        # Evolve and assign to full list
...
def mag2(x):
    """x is 1x2 np.array. Output scalar"""
    if x.shape != (2,):
...
def xdot(t, x, v):
    """xdot = v.
       v is 2x3 np.array of velocities."""
    return v
